...
const char *COMMON_FILE_PATH = "./student.c";
...
{
    printf("\n");
    semOperation(0, +1, SEM_UNDO, false); //decrease the first semaphore to indicate that this process ain't running anymore and other process can use this
    semOperation(SEM_SET_SIZE - 1, -1, IPC_NOWAIT, true); //decrease the last semaphore by one to signify that this process will no longer use the semaphore set
    print_sem_set();
...
{
    // if additional parameter is not passed throw error
    if (argc != 2)
    {
    ...
    if (!is_sem_set_initialized())
    {
        ...
        setValArg.val = 3; //max three child present
        semctl(semid, 0, SETVAL, setValArg); // set three in 0th index to set the value of semaphore for actual task
    }
    else
        printf("Semaphore set already initialized\n");

    semOperation(SEM_SET_SIZE - 1, +1, IPC_NOWAIT, true); // add to the last element to say that this process is using the semaphore set

    print_sem_set();

    semOperation(0, -1, SEM_UNDO, false); // decrease to the first semaphore to either wait till this process gets chance or continue it's work
    while (true)
    {
        printf("I am student %s!!\n", argv[1]);
        sleep(1);
    }
}